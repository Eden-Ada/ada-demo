...
const IconForSelection: React.FC<{ kind: SelectionKind }> = ({ kind }) => {
  if (kind === 'automation') {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  if (kind === 'manual') {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.15" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
const SelectionTooltip: React.FC<SelectionTooltipProps> = ({ open, anchorEl, onSelect, onClose, current }) => {
  ...
  const content = (
    <div
      ...
      role="dialog"
      aria-modal="false"
      onMouseDown={() => console.log('[SelectionTooltip] panel mousedown')}
      onClick={() => console.log('[SelectionTooltip] panel click')}
      onAnimationEnd={() => {
        if (isExiting) {
          setRendered(false)
          setIsExiting(false)
                ...
                {prevIdx !== null && (
                  ...
                  >
                    <div ref={prevCircleRef} className="selection-tooltip__circle" data-kind={OPTIONS[prevIdx].key}>
                      <div className="selection-tooltip__center-icon">
