...
const ZoomSlider: React.FC<ZoomSliderProps> = ({ min = -1.25, max = 1.25, defaultValue = 0, onChange }) => {
  ...
  const setFromClientY = (clientY: number) => {
    ...
    const v = ratioToValue(1 - r) // invert so top = max
    setValue(v)
    notify(v)
  }
  ...
  const thumbOffset = (1 - ratio) * 100 // % from top

  return (
    <div className="zoom-slider" role="group" aria-label="Zoom slider">
      <div
        ref={trackRef}
